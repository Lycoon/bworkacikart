...
public static class Matrix
{
    ...
    {
        // transpose m x n matrix into n x m matrix
        int mRows = matrix.Length; int mCols = matrix[0].Length;
        double[][] result = MatrixCreate(mCols, mRows);
        for (int i = 0; i < mRows; ++i) // each row of matrix
            for (int j = 0; j < mCols; ++j) // each col of matrix
    ...
    public static bool MatrixAreEqual(double[][] matrixA, double[][] matrixB, double epsilon)
    {
        ...
        if (aRows != bRows || aCols != bCols)
            throw new System.Exception("Non-conformable matrices");

        for (int i = 0; i < aRows; ++i) // each row of A and B
            for (int j = 0; j < aCols; ++j) // each col of A and B
                // matrixA[i][j] != matrixB[i][j]
                if (Mathf.Abs((float)(matrixA[i][j] - matrixB[i][j])) > epsilon)
                    return false;
        return true;
    }
    ...
    public static double[] MatrixVectorProduct(double[][] matrix, double[] vector)
    {
        ...
        if (mCols != vRows)
            throw new System.Exception("Non-conformable matrix and vector");
    ...
    public static double[][] MatrixDecompose(double[][] matrix, out int[] perm, out int toggle)
    {
        ...
        if (rows != cols)
            throw new System.Exception("Attempt to decompose a non-square m");
        ...
        {
            double colMax = Mathf.Abs((float)(result[j][j])); // find largest val in col
            int pRow = j;

            // reader Matt V needed this:
            for (int i = j + 1; i < n; ++i)
            {
                if (Mathf.Abs((float)(result[i][j])) > colMax)
                {
                    colMax = Mathf.Abs((float)(result[i][j]));
                    pRow = i;
                }
            }
            ...
            if (result[j][j] == 0.0)
            {
                ...
                if (goodRow == -1)
                    throw new System.Exception("Cannot use Doolittle's method");
            ...
            //if (Mathf.Abs(result[j][j]) < 1.0E-20) 
